...
{
	void *ptr = sce_paf_private_malloc(size);
	return ptr;
}
...
static int ZitByte(FILE *file, int *pi)
{
	...
	int err = sce_paf_private_fread(&c, 1, 1, file);
...
static unsigned long ZipLocateCentralDir(FILE *file)
{
	...
	if(sce_paf_private_fseek(file, 0, SEEK_END) != 0)
		return 0;

	usizefile = sce_paf_private_ftell(file);
	...
	while(ubackread < umaxback)
	{
		...
		if(sce_paf_private_fseek(file, ureadpos, SEEK_SET) != 0)
			break;

		if(sce_paf_private_fread(buf, (unsigned int)ureadsize, 1, file) != 1)
			break;
...
static int ZitZipFileInfoInternal(Zip* file, zipFileInfo *pfileinfo, zipFileInternalInfo *pfileinfointernal, char *filename, unsigned long filenamebuffersize, void *extrafield, unsigned long extrafieldbuffersize, char *comment, unsigned long commentbuffersize)
{
	...
	if(sce_paf_private_fseek(s->file, s->posincentraldir + s->bytebeforezip, SEEK_SET) != 0)
		err = _ZIP_ERRNO;
	...
	if((err == _ZIP_OK) && (filename != NULL))
	{
		...
		{
			if(sce_paf_private_fread(filename, (unsigned int)usizeread, 1, s->file) != 1)
				err = _ZIP_ERRNO;
		}
	...
	if((err == _ZIP_OK) && (extrafield != NULL))
	{
		...
		if(lseek != 0)
		{
			if(sce_paf_private_fseek(s->file, lseek, SEEK_CUR) == 0)
				lseek = 0;
			else
				err = _ZIP_ERRNO;
		}
		...
		{
			if(sce_paf_private_fread(extrafield, (unsigned int)usizeread, 1, s->file) != 1)
				err = _ZIP_ERRNO;
		}
	...
	if((err == _ZIP_OK) && (comment != NULL))
	{
		...
		{
			if(sce_paf_private_fread(comment, (unsigned int)usizeread, 1, s->file) != 1)
				err = _ZIP_ERRNO;
		}
...
static int ZipLocateFile(Zip* file, const char *filename, int casesensitive)
{
	...
	if(sce_paf_private_strlen(filename) >= _ZIP_MAX_FILENAME_SIZE)
		return _ZIP_PARAM_ERROR;
	...
	while(err == _ZIP_OK)
	{
		...
		if(sce_paf_private_strcmp(currentfilename, filename) == 0)
			return _ZIP_OK;
...
static int ZipCheckCurrentFileCoherencyHeader(_zip *s, unsigned int *pisizevar, unsigned long *poffsetstaticextrafield, unsigned int *psizestaticextrafield)
{
	...
	*psizestaticextrafield = 0;
	...
	if(sce_paf_private_fseek(s->file, s->currentfileinfointernal.currentfileoffset + s->bytebeforezip, SEEK_SET) != 0)
		return _ZIP_ERRNO;
...
static int ZipOpenCurrentFile(Zip* file, const char *password)
{
	...
	if(password != NULL)
	{
		...
		if(sce_paf_private_fseek(pfileinzipreadinfo->file, s->currentzipfileinfo->posinzip + s->currentzipfileinfo->bytebeforezip, SEEK_SET) != 0)
		{
			FreePatch(pfileinzipreadinfo->buffer);
			FreePatch(pfileinzipreadinfo);
		...
		if(sce_paf_private_fread(source, 1, 12, pfileinzipreadinfo->file) < 12)
		{
			FreePatch(pfileinzipreadinfo->buffer);
			FreePatch(pfileinzipreadinfo);
...
static int ZipReadCurrentFile(Zip* file, void* buf, unsigned int len)
{
	...
	while(pfileinzipreadinfo->stream.avail_out > 0)
	{
		if((pfileinzipreadinfo->stream.avail_in == 0) && (pfileinzipreadinfo->restreadcompressed > 0))
		{
			...
			if(sce_paf_private_fseek(pfileinzipreadinfo->file, pfileinzipreadinfo->posinzip + pfileinzipreadinfo->bytebeforezip, SEEK_SET) != 0)
				return _ZIP_ERRNO;

			if(sce_paf_private_fread(pfileinzipreadinfo->buffer, ureadthis, 1, pfileinzipreadinfo->file) != 1)
				return _ZIP_ERRNO;
...
Zip* ZipOpen(const char *filename)
{
	...
	file = sce_paf_private_fopen(filename, "rb");
	...
	if(sce_paf_private_fseek(file, centralpos, SEEK_SET) != 0)
		err = _ZIP_ERRNO;
...
void mkdir_rec(const char* dir) {
	...
	len = sce_paf_private_strlen(tmp);
	if (tmp[len - 1] == '/')
		tmp[len - 1] = 0;
	for (p = tmp + 1; *p; p++)
		if (*p == '/')         {
...
int ZipExtractCurrentFile(Zip *zip, int *nopath, const char *password, const char* path)
{
	...
	else
	{
		...
		*(filenameWithoutPath - 1) = '\0';
		...
		fout = sce_paf_private_fopen(extract_path, "wb");
		if (fout == NULL) {
		...
		do
		{
			...
			if(err > 0)
			{
				...
				if (remainingSize == 0){
					sce_paf_private_fwrite(buffer, 1, buffersize, fout);
		...
		if (remainingSize != buffersize) sce_paf_private_fwrite(buffer, 1, buffersize-remainingSize, fout);
		sce_paf_private_fclose(fout);
...
int ZipExtract(Zip* zip, const char *password, const char* path)
{
	unsigned int i;
	zipGlobalInfo gi;
	sce_paf_private_memset(&gi, 0, sizeof(zipGlobalInfo));
	int err;
	int nopath = 0;
...
void ZipFilesce_paf_private_free(ZipFile* file)
{
	if(file->data)
		FreePatch(file->data);
